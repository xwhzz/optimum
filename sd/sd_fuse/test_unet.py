...
parser.add_argument('--choice', choices=['correct', 'performance'], default='performance')
args = parser.parse_args()
WARMUP = 3
if args.choice == 'performance':
...
if __name__ == '__main__':
    ...
    data = [np.random.random((5,4,64,64)).astype(np.float16),
            np.array([70,60,50,40,30]).astype(np.int64),
            np.random.random((5,77,768)).astype(np.float16)]
    test_res(model_path, [3,2],data)
